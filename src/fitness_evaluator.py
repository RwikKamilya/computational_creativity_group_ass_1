...
class RecipeFitness:
    ...
    def __init__(self, ingredients_index: dict, overall_config: dict,
                 flavor_pairings: list, category_id: str = "cookie",
                 weights = None) -> None:
        ...
        self._reference_recipes = []

    @classmethod
    def from_files(cls, ingredients_path: str, overall_cfg_path: str, flavor_pairings_path: str,
                   category_id: str = "cookie", weights = None) -> "RecipeFitness":
    ...
    def set_reference(self, recipes: list) -> None:
    ...
    def compute_aspects(self, recipe: Recipe) -> dict:
        ...
        flavor = 0.4 * pairing + 0.6 * soft

        return {"valid": valid, "problems": problems, "ratio_score": self.ratio_fit_score(ing),
                   "pairing_score": pairing, "soft_rules_score": soft,
                   "novelty_score": self.novelty_score(ing, self._reference_recipes),
                   "simplicity_score": self.simplicity_score(ing), "flavor_score": flavor}
    ...
    def recipe_validity(self, ingredients_g: dict) -> tuple:
        problems = []
    ...
    def novelty_score(self, target_ing: dict, all_recipes: list) -> float:
    ...
    def _has_any(self, ing_ids: set, keys: list) -> bool:
        return any(k in ing_ids for k in (keys or []))

    def _grams_of_any(self, ing_g: dict, keys: list) -> float:
    ...
    def _score_threshold_preference(self, ing_g: dict, rule: dict) -> float:
        any_of = rule.get("any_of", [])
        min_grams = float(rule.get("min_grams", 0.0))
        ok = any(float(ing_g.get(k, 0.0)) >= min_grams for k in any_of)
        return 1.0 if ok else 0.0

    def _score_diversity_preference(self, ing_g: dict, rule: dict) -> float:
    ...
    def soft_rules_score(self, ingredients_g: dict) -> float:
        ...
        for r in self.soft_rules:
            ...
            if local_score is not None:
                total_score += self.clamp01(local_score) * w
                total_weight += w

        return float(total_score / total_weight) if total_weight > 0 else 0.0

    @staticmethod
    def simplicity_score(ingredients_g: dict,
                         ideal_lo: int = 6,
                         ideal_hi: int = 12,
                         min_n: int = 3,
                         max_n: int = 20) -> float:
